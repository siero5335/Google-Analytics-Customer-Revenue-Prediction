...
for (grp in c("wday", "hour", "month", "week", "qday", "weekofmonth", "minute", "hour2")) {
...
tr_te_ohe <- tr_te %>%
  select(-date, -fullVisitorId, -visitId, visitStartTime) %>% 
  select_if(is.character) %>% 
  mutate_all(factor) 

nzv <- nearZeroVar(tr_te_ohe)

tr_te_ohe <- tr_te_ohe[, -nzv]
...
rm(tr, te, grp, col, flatten_json, parse, has_many_values, tr_te_ohe,
   is_na_val, fn, df, s)
gc(); gc()
...
dtrain <- lgb.Dataset(data.matrix(tr_te[idx, ]), label = y[idx, ])

dval <-  lgb.Dataset(data.matrix(tr_te[!idx, ]), label = y[!idx, ])
...
model1_1 <- lgb.train(data = dtrain, valids = list(eval = dval), params = params, 
