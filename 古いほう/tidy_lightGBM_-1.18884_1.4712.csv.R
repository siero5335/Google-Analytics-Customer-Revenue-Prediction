...
rm(tr, te, grp, col, flatten_json, parse, has_many_values, is_na_val,
   ...
   sum_by_month, sum_by_ref, sum_by_source, sum_by_vn, tr_te_ohe, sum_by_qday, sum_by_qdayofyear, sum_by_wday)
gc(); gc()
...
dtrain <- lgb.Dataset(data.matrix(tr_te[idx, ]), label = log1p(y[idx]))

dval <-  lgb.Dataset(data.matrix(tr_te[-idx, ]), label = log1p(y[-idx]))
...
model1_1 <- lgb.train(data = dtrain, valids = list(eval = dval), params = params, 
                      nrounds = 3000, early_stopping_rounds = 100, eval_freq = 50)
