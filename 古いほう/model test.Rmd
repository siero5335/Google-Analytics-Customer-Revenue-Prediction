...
dftest <- read_csv("dftest.csv", na = c(NA, "not available in demo dataset", "(not set)"))
```
...
dftrainCV5 <- dftrainCV5[, -which(colMeans(is.na(dftrainCV5)) > 0.9)]
dftrainRes5 <- dftrainRes5[, -which(colMeans(is.na(dftrainRes5)) > 0.9)]
dftest <- dftest[, -which(colMeans(is.na(dftest)) > 0.9)]
...
```{r}
dftrainCV5 <- dftrainCV5 %>% select(-rem)
dftrainRes5 <- dftrainRes5 %>% select(-rem)
dftest <- dftest%>% select(-rem)
...
#model1
set.seed(71)
X1 <- lgb.Dataset(data.matrix(dftrainCV5 %>% select(-transactionRevenue, -date, -fullVisitorId, -visitId, -sessionId, -transactionRevenue2)), 
                  label = as.numeric(dftrainCV5$transactionRevenue))

evalX1 <-  lgb.Dataset(data.matrix(dftrainRes5 %>% select(-transactionRevenue, -date, -fullVisitorId, -visitId, -sessionId, -transactionRevenue2)), 
...
```{r}
pred <- predict(model1_1, data.matrix(dftrainRes5 %>% select(-transactionRevenue, -transactionRevenue2, -date, -fullVisitorId, -visitId, -sessionId)), n = model1_1$best_iter) 
```
