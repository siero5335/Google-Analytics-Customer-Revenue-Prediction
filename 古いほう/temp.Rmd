...
tr_te <- tr_te %>% 
  ...
  mutate(weekofyear = factor(paste(year, weekdays, sep = "_")),
         weekofmonth = factor(paste(year, month, sep = "_")),
         weekofday = factor(paste(year, day, sep = "_")),
         transactionRevenue = ifelse(is.na(transactionRevenue), 0, transactionRevenue),
...
```{r}  
tr_te$pageviews[is.na(tr_te$pageviews)]<-0

tr_te <- tr_te %>%
  mutate(source_country = factor(paste(source, country, sep = "_")),
         campaign_medium = factor(paste(campaign, medium, sep = "_")),
         browser_category = factor(paste(browser, deviceCategory, sep = "_")),
         browser_os = factor(paste(browser, operatingSystem, sep = "_")))  %>% 
         mutate_if(is.character, funs(factor(.) %>% as.integer)) %>%
         mutate_if(is.logical, funs(factor(.) %>% as.integer)) %>%
